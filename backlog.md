# BACKLOG

## ENTREGA 21 NOVIEMBRE 2025

Pendientes para poder finalizar el desarrollo:
* Urls de los ambientes para el widget / pendiente
* Icono del widget  / ok
* Logo del sistema  / ok
* Nombre par el chat web  IDARTES / ok
* Mensaje de saludo  / ok
* Mensaje de despedida  / ok
* Mensaje desiste terminos y condiciones  / ok
* Ajustar arbol vs encuesta para no preguntar lo mismo dos veces y hacer llegar tanto arbol final como encuesta final que debe quedar.  / ok

* Colores de letras - libre a nuestro diseño / ok
* Fuentes de letra - libre a nuestro diseño / ok
* Color de fondo - libre a nuestro diseño / ok

* Repositorio / ok
* A que servicio nos conectamos o como se manejara el tema de paso agente. / pendiente que funcione el core y desarrollar para garantizar comunicacion con widget, interacion soul - widget


• Imagen corporativa.
• Arbol para el widget.
• Crear base de datos.
• Crear base inicial.