...
// * FORMULARIO INICIAL - WIDGET IDARTES
const formularioInicial = async (
    idChatWeb,
    pasoArbol,
    nombresApellidos,
    genero,
    ...
    try {
        connMySQL = await pool.getConnection();
        // Actualizar solo los campos nuevos del formulario inicial
        const query = `
            UPDATE tbl_chat SET
                cht_arbol = ?,
            ...
            WHERE cht_remitente = ?;
        `;
        const result = await connMySQL.query(query, [pasoArbol,
            nombresApellidos,
            genero,
            correoElectronico,
            telefono,
...
const actualizar = async (idChat, pasoArbol, chatData) => {
    ...
    try {
        ...
        const params = [
            ...
            safeString(chatData.temaConsulta),
            safeString(chatData.rangoEdad),
            safeString(chatData.autorizacionDatosPersonales),
